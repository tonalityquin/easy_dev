...
class RpsImageClassifierResult {
  const RpsImageClassifierResult({
  ...
  static const RpsImageClassifierResult empty = RpsImageClassifierResult(
    success: false,
    label: null,
    displayLabel: null,
    confidence: null,
    ...
    minAutoConfidence: RpsImageClassifier.minAutoConfidence,
    minAutoMargin: RpsImageClassifier.minAutoMargin,
...
class RpsImageClassifier {
  RpsImageClassifier._internal();

  static final RpsImageClassifier instance = RpsImageClassifier._internal();

  static const String modelAssetPath = 'assets/models/test_small_model/rps_classifier.tflite';
  static const String labelsAssetPath = 'assets/models/test_small_model/labels.txt';
  static const int inputSize = 224;
  static const double minAutoConfidence = 0.70;
  static const double minAutoMargin = 0.20;
  static const String inputMode = 'full_frame';
  static const List<String> fallbackLabels = ['paper', 'rock', 'scissors'];
  ...
  Future<RpsImageClassifierResult> classifyFile(String path) async {
    final logs = <String>[];
    ...
    try {
      ...
      if (!file.existsSync()) {
        logs.add('RPS 이미지 파일 없음 path=$path');
        return _failureResult(
          logs: logs,
      ...
      logs.add('RPS 이미지 로드 mode=$inputMode bytes=${bytes.length}');
      final decoded = img.decodeImage(bytes);
      if (decoded == null) {
        logs.add('RPS 이미지 디코딩 실패');
        return _failureResult(
          logs: logs,
      ...
      logs.add('RPS 전처리 inputMode=$inputMode resize=${inputSize}x$inputSize channels=RGB normalize=0..1');
      ...
      logs.add(
        'RPS 추론 완료 top1=${topLabel ?? '-'} ${_formatProbability(topConfidence)} '
      ...
      return RpsImageClassifierResult(
        success: true,
        label: topLabel,
  ...
  Future<void> _ensureLoaded(List<String> logs) async {
    ...
    try {
      ...
      logs.add('RPS 모델 로드 완료 asset=$modelAssetPath bytes=${modelBytes.length}');
      ...
      logs.add('RPS 라벨 로드 완료 labels=${_labels!.join(', ')}');
    } finally {
      _loading = false;
    }
  }

  RpsImageClassifierResult _failureResult({
