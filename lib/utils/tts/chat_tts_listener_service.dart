...
// - 이후 더 최신인 경우에만 낭독
// - read 카운터(추정) + UsageReporter 계측 추가: 초기 스냅샷(있으면 1), 이후 스냅샷마다 1
//
// *latest_message*는 단일 문서 리스닝이므로 초기/이후 각각 1 read로 추정합니다.
// (이 파일은 Firestore "읽기(read)"만 발생하므로 UsageReporter도 read만 기록합니다.)
    ...
    _subscription = FirebaseFirestore.instance
      ...
      if (_skipFirstSnapshot) {
        ...
        if (docSnapshot.exists) {
          ...
          _reportRead(n: 1, source: 'ChatTtsListenerService.snap.initial');
        }
      } else {
        // 이후 스냅샷: 변경이 올 때마다 1 read로 추정
        _readsUpdates += 1;
        debugPrint('[ChatTTS] READ++ (update) → $_readsUpdates');
        _reportRead(n: 1, source: 'ChatTtsListenerService.snap.update');
      }
