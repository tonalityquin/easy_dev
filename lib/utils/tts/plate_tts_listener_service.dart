...
import '../../features/plate/data/repositories/firestore_plate_tts_listener_repository.dart';
import '../../features/plate/domain/enums/plate_type.dart';
import '../../features/plate/domain/repositories/plate_tts_listener_repository.dart';
...
import 'plate_tts_event_hub.dart';
...
class PlateTtsListenerService {
  ...
  static Future<void> _hydrateFromPrefsSafe(
    ...
    try {
      ...
      _log('hydrate(prefs) reason=$reason filters=${loaded
          .toMap()} masterOn=$masterOn');
    } catch (e, st) {
  ...
  static Future<void> _applyEffectiveMasterPolicy(
    ...
    try {
      ...
      if (_subscription == null && (_lastKnownArea ?? '')
          .trim()
          .isNotEmpty) {
        ...
        _log(
            'policy($reason): effective master ON (subscription=${_subscription !=
                null})');
      }
    } finally {
      _policyBusy = false;
  ...
  static Future<void> _startListening(String currentArea,
    ...
    _log(
        'listen config: area=$area mode=$mode isTablet=$isTablet types=${typesToMonitor
            .map((e) => e.firestoreValue).toList()} (seq=$mySeq)');

    try {
      await _fetchBaseline(area, typesToMonitor);

      if (_baselineUpdatedAt != null && _baselineDocId != null) {
        _log('apply cursor(startAfter): ts=${_baselineUpdatedAt
            ?.toUtc()} id=$_baselineDocId (seq=$mySeq)');
      } else {
      ...
      _subscription = _repository
        ...
        _readsTotal += 1;
        _readsAdded += docChanges
            .where((c) => c.type == PlateTtsChangeType.added)
            .length;
        ...
        _log('snapshot changes=${docChanges
            .length}, fromCache=$isFromCache (seq=$mySeq)');
        ...
        for (final change in docChanges) {
          ...
          final tail = plateNumber.length >= 4 ? plateNumber.substring(
              plateNumber.length - 4) : plateNumber;
          ...
          if (change.type == PlateTtsChangeType.added) {
            if (_dedup(docId)) {
              if (newType == PlateType.parkingRequests.firestoreValue) {
                ...
                _safeSpeak(utter);
                _notifyPlateEvent(docId: docId,
                    type: newType,
                    plateNumber: plateNumber,
                    location: location);
                didSpeak = true;
              } else
              if (newType == PlateType.departureRequests.firestoreValue) {
              ...
              if (newType == PlateType.departureCompleted.firestoreValue) {
                ...
                _notifyPlateEvent(docId: docId,
                    ...
                    location: location);
                _speakRepeated(
                    utter, times: _completionRepeat, gap: _completionRepeatGap);
                didSpeak = true;
              } else {
  ...
  static Future<void> _fetchBaseline(String currentArea,
      List<PlateType> types) async {
    try {
  ...
  static String _readPlateNumber(Map<String, dynamic> data) {
    ...
    if (v1 is String && v1
        .trim()
        .isNotEmpty) return v1.trim();

    final v2 = data['plate_number'];
    if (v2 is String && v2
        .trim()
        .isNotEmpty) return v2.trim();
  ...
  static String _readLocationForSpeech(dynamic raw) {
    ...
    if (raw is Map) {
      final full = raw['full'];
      if (full is String && full
          .trim()
          .isNotEmpty) return full.trim();

      final leaf = raw['leaf'];
      if (leaf is String && leaf
          .trim()
          .isNotEmpty) return leaf.trim();
  ...
  static void _reportUsageRead({
    ...
    if (_usageSampleRate < 1.0 && sampled) {
      final r = (DateTime
          .now()
          .microsecondsSinceEpoch % 1000) / 1000.0;
