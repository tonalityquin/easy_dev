...
//    * 예외는 상위 호출부로 그대로 던지는 동작 유지
// - ❌ /reports 관련 업로드 로직 완전 제거:
//    * uploadEndWorkReportJson 삭제(미사용/정책상 제거)
// - ✅ (추가) /logs 경로에 월(yyyyMM) 디렉터리를 $ts 이전에 추가:
//    * <division>/<area>/logs/<yyyyMM>/<ts>/<fileName>
//    * 월 단위 관리(탐색/정리/라이프사이클) 용이
...
///
/// - [json]: 업로드할 JSON Map
/// - [destinationPath]: GCS object name (예: "division/area/logs/xxx.json")
/// - [purpose]: 로깅용 설명 문자열 (예: "업무 종료 로그")
/// - [makePublicRead]: publicRead ACL 적용 여부
///
/// invalid_token 방어 로직:
/// - 1차 시도에서 invalid_token 감지 시 바깥에서 refreshIfNeeded() 수행 후 1회 재시도
/// - 2차 시도까지 실패하면 예외는 그대로 상위로 던짐(기존 호출부 try/catch와 호환)
Future<gcs.Object> _uploadJsonToGcs({
  ...
  Future<gcs.Object> runOnce({required bool allowRethrowInvalid}) async {
    ...
    try {
      ...
      // 그 외 예외는 그대로 상위로 던져서 기존 호출부에서 처리하도록 유지
      rethrow;
    } finally {
      if (temp != null) {
        try {
...
///   GcsJsonUploader.loadPlateLogs 의 불러오기 로직과 100% 호환(파일명 기준).
/// - 상위 경로:
///   <division>/<area>/logs/<yyyyMM>/<timestamp>/
///   (월 단위 관리 + 재업로드 충돌 방지)
///
/// - 예(버킷 포함 표기):
///   easydev-image/belivus/가로수길(캔버스랩)/logs/202512/1766133101506/
///   조현수_1766133101506_ToDoLogs_2025-12-19.json
///
/// - 반환:
///   성공 시 public URL
...
Future<String?> uploadEndLogJson({
  ...
  // ✅ 월 샤딩 키(yyyyMM) 추가: ts 이전에 연월 디렉터리 삽입
  final monthKey = _yyyyMM(now);

  // ✅ 파일명 접미사 규칙 고정: "_ToDoLogs_YYYY-MM-DD.json"
  final fileName = '${safeUser}_${ts}_ToDoLogs_${dateStr}.json';

  // ✅ logs/<yyyyMM>/<timestamp>/ 하위 경로
  ...
  return res.name != null ? 'https://storage.googleapis.com/$kBucketName/${res.name}' : null;
}
