...
class _NormalSelectionBaseline {
  final bool isSelected;
  final String? selectedBy;

  const _NormalSelectionBaseline({required this.isSelected, required this.selectedBy});
}

/// ✅ 1회 조회 결과(문서 + 소스) 묶음
class _NormalFetchResult {
  final List<PlateModel> items;
  final String sourceLabel; // 'server' | 'cache'
  const _NormalFetchResult({required this.items, required this.sourceLabel});
}

class NormalPlateState extends ChangeNotifier {
  /// ✅ Normal 모드에서는 "입차 완료/출차 완료"만 사용(데이터 대상 제한)
  static const Set<PlateType> NormalAllowedTypes = {
  ...
  final Map<PlateType, DateTime?> _lastNormalRefreshAtByType = {
    for (final t in PlateType.values) t: null,
  };
  final Map<PlateType, String> _lastNormalRefreshSourceByType = {
    for (final t in PlateType.values) t: '-',
  };

  DateTime? normalLastRefreshAtOf(PlateType type) => _lastNormalRefreshAtByType[type];

  String normalLastRefreshSourceLabelOf(PlateType type) => _lastNormalRefreshSourceByType[type] ?? '-';

  /// Normal에서도 기존 로직 호환을 위해 유지
  ...
  final Map<String, _NormalSelectionBaseline> _baseline = <String, _NormalSelectionBaseline>{};
  ...
  NormalPlateState(this._repository, this._areaState) {
  ...
  // ─────────────────────────────────────────────────────────────
  // 공개 스위치: Normal 화면에서만 데이터 로드 활성화
  // ─────────────────────────────────────────────────────────────

  /// Normal 모드: withDefaults=true면 "입차완료/출차완료" 2종을 1회 조회로 로드합니다.
  /// (중요) 여기서 “구독”은 절대 하지 않습니다.
  void normalEnableForTypePages({bool withDefaults = true}) {
    if (_enabled) return;
    _enabled = true;

    debugPrint('🔔 [Normal] NormalPlateState enabled (NO-SUBSCRIBE) / withDefaults=$withDefaults');
  ...
  void normalDisableAll() {
    if (!_enabled && _activeTypes.isEmpty) return;

    _enabled = false;
    _lifecycleEpoch++; // 진행 중 로드 결과 무시
    debugPrint('🔕 [Normal] NormalPlateState disabled (NO-SUBSCRIBE)');
    ...
    for (final t in PlateType.values) {
      ...
      _reqSeqByType[t] = 0;
      _lastNormalRefreshAtByType[t] = null;
      _lastNormalRefreshSourceByType[t] = '-';
    }
  ...
  void normalSubscribeType(PlateType type) {
    if (!NormalAllowedTypes.contains(type)) {
      debugPrint('🚫 [Normal] subscribeType ignored (not allowed): $type');
      return;
    }
    if (!_enabled) {
      debugPrint('🔕 [Normal] disabled → subscribeType ignored: $type');
      return;
    }
    ...
    unawaited(normalRefreshType(type));
  }

  /// Area 변경 감지 시: 활성 타입들만 1회 재조회
  void normalSyncWithAreaState() {
    if (!_enabled) {
      debugPrint('🔕 [Normal] disabled → syncWithAreaState ignored');
      return;
    }

    debugPrint('🔄 [Normal] syncWithAreaState (NO-SUBSCRIBE) → refresh active types');

    _baseline.clear();
    _clearPendingSelectionInternal();

    for (final t in _activeTypes.toList()) {
      unawaited(normalRefreshType(t));
    }
  }

  void _initDefaultLoads() {
    // Normal 기본: 입차 완료 + 출차 완료
    normalSubscribeType(PlateType.parkingCompleted);
    normalSubscribeType(PlateType.departureCompleted);
  }
  ...
  List<PlateModel> _parseSnapshot(QuerySnapshot<Map<String, dynamic>> snap, PlateType type) {
    ...
    for (final doc in snap.docs) {
      try {
        ...
        debugPrint('❌ [Normal] parse error: type=$type, doc=${doc.id}, err=$e');
      }
    }
    return results;
  }
  ...
  /// ✅ 서버 우선 조회 + 실패 시 캐시 폴백
  Future<_NormalFetchResult> _getOnce({
    required PlateType type,
    required String area,
    required bool descending,
    ...
    try {
      ...
      debugPrint('🌐 [Normal][${_getTypeLabel(type)}] server get: ${serverResults.length}개 (area=$area)');
      return _NormalFetchResult(items: serverResults, sourceLabel: 'server');
    } catch (e) {
      debugPrint('⚠️ [Normal][${_getTypeLabel(type)}] server get 실패 → ${cacheFirst ? 'cache로 폴백' : '종료'}: $e');
      ...
      debugPrint('💾 [Normal][${_getTypeLabel(type)}] cache get: ${cacheResults.length}개 (area=$area)');
      return _NormalFetchResult(items: cacheResults, sourceLabel: 'cache');
    }
  }

  Future<void> normalRefreshType(PlateType type) async {
    if (!_enabled) return;
    if (!NormalAllowedTypes.contains(type)) return;
    ...
    debugPrint('🔎 [Normal][${_getTypeLabel(type)}] 1회 로드 시작 (area=$area, desc=$descending)');
    ...
    try {
      ...
      _lastNormalRefreshAtByType[type] = DateTime.now();
      _lastNormalRefreshSourceByType[type] = fetched.sourceLabel;
        ...
        for (final removedId in removedIds) {
          try {
            ...
            if (sameArea && isDepartureCompleted && isLockedFeeTrue) {
              ...
              debugPrint('✅ [Normal] 정산 전이 감지(1회 조회 비교): id=$removedId, plate=${fresh.plateNumber}');
            }
          } catch (e) {
            debugPrint('⚠️ [Normal] removed 후속 확인 실패: $e');
          }
        }
      }
      ...
      for (final p in results) {
        ...
        _baseline[p.id] = _NormalSelectionBaseline(
      ...
      if (hasPendingSelection && !pendingStillValidFor(type)) {
        _clearPendingSelectionInternal();
        notifyListeners();
        debugPrint('ℹ️ [Normal] 외부 변경/갱신으로 보류 선택을 해제했습니다.');
      }

      debugPrint('✅ [Normal][${_getTypeLabel(type)}] 1회 로드 완료: ${results.length}개');
    } catch (e) {
      debugPrint('🔥 [Normal][${_getTypeLabel(type)}] 1회 로드 실패: $e');
    } finally {
      // 토큰이 살아있을 때만 로딩 해제
  ...
  PlateModel? normalGetSelectedPlate(PlateType collection, String userName) {
  ...
  Future<void> normalTogglePlateIsSelected({
  ...
  List<PlateModel> normalGetPlatesByCollection(PlateType collection, {DateTime? selectedDate}) {
  ...
  Future<void> normalUpdatePlateLocally(PlateType collection, PlateModel updatedPlate) async {
  ...
  void _onAreaChanged() {
    ...
    debugPrint('🔄 [Normal] area changed → refresh active types (NO-SUBSCRIBE)');
