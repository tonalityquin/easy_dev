...
class _DoubleSelectionBaseline {
  final bool isSelected;
  final String? selectedBy;

  const _DoubleSelectionBaseline({required this.isSelected, required this.selectedBy});
}

/// ✅ 1회 조회 결과(문서 + 소스) 묶음
class _DoubleFetchResult {
  final List<PlateModel> items;
  final String sourceLabel; // 'server' | 'cache'
  const _DoubleFetchResult({required this.items, required this.sourceLabel});
}

class DoublePlateState extends ChangeNotifier {
  /// ✅ Double 모드에서는 "입차 완료/출차 완료"만 사용(데이터 대상 제한)
  static const Set<PlateType> doubleAllowedTypes = {
  ...
  final Map<PlateType, DateTime?> _lastDoubleRefreshAtByType = {
    for (final t in PlateType.values) t: null,
  };
  final Map<PlateType, String> _lastDoubleRefreshSourceByType = {
    for (final t in PlateType.values) t: '-',
  };

  DateTime? doubleLastRefreshAtOf(PlateType type) => _lastDoubleRefreshAtByType[type];

  String doubleLastRefreshSourceLabelOf(PlateType type) => _lastDoubleRefreshSourceByType[type] ?? '-';

  /// Double에서도 기존 로직 호환을 위해 유지
  ...
  final Map<String, _DoubleSelectionBaseline> _baseline = <String, _DoubleSelectionBaseline>{};
  ...
  DoublePlateState(this._repository, this._areaState) {
  ...
  // ─────────────────────────────────────────────────────────────
  // 공개 스위치: Double 화면에서만 데이터 로드 활성화
  // ─────────────────────────────────────────────────────────────

  /// Double 모드: withDefaults=true면 "입차완료/출차완료" 2종을 1회 조회로 로드합니다.
  /// (중요) 여기서 “구독”은 절대 하지 않습니다.
  void doubleEnableForTypePages({bool withDefaults = true}) {
    if (_enabled) return;
    _enabled = true;

    debugPrint('🔔 [Double] DoublePlateState enabled (NO-SUBSCRIBE) / withDefaults=$withDefaults');
  ...
  void doubleDisableAll() {
    if (!_enabled && _activeTypes.isEmpty) return;

    _enabled = false;
    _lifecycleEpoch++; // 진행 중 로드 결과 무시
    debugPrint('🔕 [Double] DoublePlateState disabled (NO-SUBSCRIBE)');
    ...
    for (final t in PlateType.values) {
      ...
      _reqSeqByType[t] = 0;
      _lastDoubleRefreshAtByType[t] = null;
      _lastDoubleRefreshSourceByType[t] = '-';
    }
  ...
  void doubleSubscribeType(PlateType type) {
    if (!doubleAllowedTypes.contains(type)) {
      debugPrint('🚫 [Double] subscribeType ignored (not allowed): $type');
      return;
    }
    if (!_enabled) {
      debugPrint('🔕 [Double] disabled → subscribeType ignored: $type');
      return;
    }
    ...
    unawaited(doubleRefreshType(type));
  }

  /// Area 변경 감지 시: 활성 타입들만 1회 재조회
  void doubleSyncWithAreaState() {
    if (!_enabled) {
      debugPrint('🔕 [Double] disabled → syncWithAreaState ignored');
      return;
    }

    debugPrint('🔄 [Double] syncWithAreaState (NO-SUBSCRIBE) → refresh active types');

    _baseline.clear();
    _clearPendingSelectionInternal();

    for (final t in _activeTypes.toList()) {
      unawaited(doubleRefreshType(t));
    }
  }

  void _initDefaultLoads() {
    // Double 기본: 입차 완료 + 출차 완료
    doubleSubscribeType(PlateType.parkingCompleted);
    doubleSubscribeType(PlateType.departureCompleted);
  }
  ...
  List<PlateModel> _parseSnapshot(QuerySnapshot<Map<String, dynamic>> snap, PlateType type) {
    ...
    for (final doc in snap.docs) {
      try {
        ...
        debugPrint('❌ [Double] parse error: type=$type, doc=${doc.id}, err=$e');
      }
    }
    return results;
  }
  ...
  /// ✅ 서버 우선 조회 + 실패 시 캐시 폴백
  Future<_DoubleFetchResult> _getOnce({
    required PlateType type,
    required String area,
    required bool descending,
    ...
    try {
      ...
      debugPrint('🌐 [Double][${_getTypeLabel(type)}] server get: ${serverResults.length}개 (area=$area)');
      return _DoubleFetchResult(items: serverResults, sourceLabel: 'server');
    } catch (e) {
      debugPrint('⚠️ [Double][${_getTypeLabel(type)}] server get 실패 → ${cacheFirst ? 'cache로 폴백' : '종료'}: $e');
      ...
      debugPrint('💾 [Double][${_getTypeLabel(type)}] cache get: ${cacheResults.length}개 (area=$area)');
      return _DoubleFetchResult(items: cacheResults, sourceLabel: 'cache');
    }
  }

  Future<void> doubleRefreshType(PlateType type) async {
    if (!_enabled) return;
    if (!doubleAllowedTypes.contains(type)) return;
    ...
    debugPrint('🔎 [Double][${_getTypeLabel(type)}] 1회 로드 시작 (area=$area, desc=$descending)');
    ...
    try {
      ...
      _lastDoubleRefreshAtByType[type] = DateTime.now();
      _lastDoubleRefreshSourceByType[type] = fetched.sourceLabel;
        ...
        for (final removedId in removedIds) {
          try {
            ...
            if (sameArea && isDepartureCompleted && isLockedFeeTrue) {
              ...
              debugPrint('✅ [Double] 정산 전이 감지(1회 조회 비교): id=$removedId, plate=${fresh.plateNumber}');
            }
          } catch (e) {
            debugPrint('⚠️ [Double] removed 후속 확인 실패: $e');
          }
        }
      }
      ...
      for (final p in results) {
        ...
        _baseline[p.id] = _DoubleSelectionBaseline(
      ...
      if (hasPendingSelection && !pendingStillValidFor(type)) {
        _clearPendingSelectionInternal();
        notifyListeners();
        debugPrint('ℹ️ [Double] 외부 변경/갱신으로 보류 선택을 해제했습니다.');
      }

      debugPrint('✅ [Double][${_getTypeLabel(type)}] 1회 로드 완료: ${results.length}개');
    } catch (e) {
      debugPrint('🔥 [Double][${_getTypeLabel(type)}] 1회 로드 실패: $e');
    } finally {
      // 토큰이 살아있을 때만 로딩 해제
  ...
  PlateModel? doubleGetSelectedPlate(PlateType collection, String userName) {
  ...
  Future<void> doubleTogglePlateIsSelected({
  ...
  List<PlateModel> doubleGetPlatesByCollection(PlateType collection, {DateTime? selectedDate}) {
  ...
  Future<void> doubleUpdatePlateLocally(PlateType collection, PlateModel updatedPlate) async {
  ...
  void _onAreaChanged() {
    ...
    debugPrint('🔄 [Double] area changed → refresh active types (NO-SUBSCRIBE)');
