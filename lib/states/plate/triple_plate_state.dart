...
class _TripleSelectionBaseline {
  ...
  const _TripleSelectionBaseline({required this.isSelected, required this.selectedBy});
}

/// ✅ 1회 조회 결과(문서 + 소스) 묶음
class _TripleFetchResult {
  final List<PlateModel> items;
  final String sourceLabel; // 'server' | 'cache'
  const _TripleFetchResult({required this.items, required this.sourceLabel});
}

class TriplePlateState extends ChangeNotifier {
  /// ✅ Triple 모드에서는 "입차 완료/출차 완료"만 사용(데이터 대상 제한)
  ...
  DateTime? tripleLastRefreshAtOf(PlateType type) => _lastTripleRefreshAtByType[type];

  String tripleLastRefreshSourceLabelOf(PlateType type) => _lastTripleRefreshSourceByType[type] ?? '-';

  /// Triple에서도 기존 로직 호환을 위해 유지
  ...
  final Map<String, _TripleSelectionBaseline> _baseline = <String, _TripleSelectionBaseline>{};
  ...
  // ─────────────────────────────────────────────────────────────
  // 공개 스위치: Triple 화면에서만 데이터 로드 활성화
  // ─────────────────────────────────────────────────────────────

  /// Triple 모드: withDefaults=true면 "입차완료/출차완료" 2종을 1회 조회로 로드합니다.
  /// (중요) 여기서 “구독”은 절대 하지 않습니다.
  void tripleEnableForTypePages({bool withDefaults = true}) {
    if (_enabled) return;
    _enabled = true;

    debugPrint('🔔 [Triple] TriplePlateState enabled (NO-SUBSCRIBE) / withDefaults=$withDefaults');
  ...
  void tripleSyncWithAreaState() {
    ...
    debugPrint('🔄 [Triple] syncWithAreaState (NO-SUBSCRIBE) → refresh active types');
  ...
  List<PlateModel> _parseSnapshot(QuerySnapshot<Map<String, dynamic>> snap, PlateType type) {
    ...
    for (final doc in snap.docs) {
      try {
        ...
        debugPrint('❌ [Triple] parse error: type=$type, doc=${doc.id}, err=$e');
      }
    }
    return results;
  }
  ...
  /// ✅ 서버 우선 조회 + 실패 시 캐시 폴백
  Future<_TripleFetchResult> _getOnce({
    required PlateType type,
    required String area,
    required bool descending,
    ...
    try {
      ...
      debugPrint('🌐 [Triple][${_getTypeLabel(type)}] server get: ${serverResults.length}개 (area=$area)');
      return _TripleFetchResult(items: serverResults, sourceLabel: 'server');
    } catch (e) {
      debugPrint('⚠️ [Triple][${_getTypeLabel(type)}] server get 실패 → ${cacheFirst ? 'cache로 폴백' : '종료'}: $e');
      ...
      debugPrint('💾 [Triple][${_getTypeLabel(type)}] cache get: ${cacheResults.length}개 (area=$area)');
      return _TripleFetchResult(items: cacheResults, sourceLabel: 'cache');
    }
  }
  ...
  Future<void> tripleRefreshType(PlateType type) async {
    ...
    debugPrint('🔎 [Triple][${_getTypeLabel(type)}] 1회 로드 시작 (area=$area, desc=$descending)');
    ...
    try {
        ...
        for (final removedId in removedIds) {
          try {
            ...
            if (sameArea && isDepartureCompleted && isLockedFeeTrue) {
              ...
              debugPrint('✅ [Triple] 정산 전이 감지(1회 조회 비교): id=$removedId, plate=${fresh.plateNumber}');
            }
          } catch (e) {
      ...
      debugPrint('✅ [Triple][${_getTypeLabel(type)}] 1회 로드 완료: ${results.length}개');
    } catch (e) {
  ...
  List<PlateModel> tripleGetPlatesByCollection(PlateType collection, {DateTime? selectedDate}) {
  ...
  Future<void> tripleUpdatePlateLocally(PlateType collection, PlateModel updatedPlate) async {
  ...
  void _onAreaChanged() {
    ...
    debugPrint('🔄 [Triple] area changed → refresh active types (NO-SUBSCRIBE)');
