...
class _RealTimeStatusPreviewBodyState extends State<RealTimeStatusPreviewBody>
  ...
  TypeAutoTransitionGuard? _cachedAutoGuard;
  ...
  void didChangeDependencies() {
    ...
    try {
      _cachedAutoGuard = context.read<TypeAutoTransitionGuard>();
    } catch (_) {
      _cachedAutoGuard = null;
    }
  }
  ...
  ParkingSlotStatus _effectiveStatus(
    ...
    if (status == ParkingSlotStatus.departureRequest && row.isSelected) {
  ...
  bool _isCurrentlySelected(
    ...
    if (normalizedCollection.isEmpty || area.isEmpty) return false;
    try {
      ...
      for (final current in currentRows) {
        if (plateId.isNotEmpty && current.plateId.trim() == plateId) {
  ...
  void _emitDrivingDockBlocked({
    required RealTimeRowVM row,
    required String source,
    String? collection,
  }) {
    final line =
        '[RealTimeStatusDotMap] event=status_dock_blocked reason=driving source=$source collection=${collection ?? '-'} plateId=${row.plateId} plateNumber=${row.plateNumber} selectedBy=${row.selectedBy ?? '-'} action=keep_current_context';
    debugPrint(line);
  }
  ...
  Future<void> _openStatusDock(
    ...
    if (_isCurrentlySelected(row, collection)) {
      _emitDrivingDockBlocked(
        row: row,
  ...
  Future<void> _openStatusDockWithSpec({
    ...
    try {
      ...
      trace = await DeveloperOperationTrace.start(
        context: context,
        title: '현황 상태 빠른 실행',
        initialMessage: '현황 주차 지도 상태 처리 빠른 실행을 준비했습니다.',
        useCommonUi: true,
