...
class MyTaskHandler implements TaskHandler {
  ...
  Future<void> onStart(DateTime timestamp, TaskStarter starter) async {
    ...
    if (f != null) {
      ...
      final masterOn =
          (isTablet ? f.departure : (f.parking || f.departure)) || completedOk;
      _listeningMode = mode;
      ...
      debugPrint(
          '[HANDLER][${_ts()}] bootstrap filters applied: ${f.toMap()} masterOn=$masterOn');
    }
  }
  ...
  void onReceiveData(dynamic data) async {
    ...
    if (incomingFilters != null) {
      ...
      masterOn = (isTablet
              ? incomingFilters.departure
              : (incomingFilters.parking || incomingFilters.departure)) ||
          completedOk;
      ...
      debugPrint(
          '[HANDLER][${_ts()}] filters applied in FG: ${incomingFilters.toMap()} masterOn=$masterOn');
    }
    ...
    if (_listeningArea != area || modeChanged) {
      ...
      debugPrint(
          '[HANDLER][${_ts()}] start listening in FG isolate: area="$area" mode="$modeForListen" (prevArea=$prevArea prevMode=$prevMode)');
      PlateTtsListenerService.start(area, force: true);
      return;
    }

    debugPrint(
        '[HANDLER][${_ts()}] same area="$area" mode="$modeForListen" → no-op');
  }
