...
class MinorPlateState extends ChangeNotifier {
  ...
  DateTime? minorLastRefreshAtOf(PlateType type) =>
      _lastMinorRefreshAtByType[type];

  String minorLastRefreshSourceLabelOf(PlateType type) =>
      _lastMinorRefreshSourceByType[type] ?? '-';
  ...
  final Map<String, _MinorSelectionBaseline> _baseline =
      <String, _MinorSelectionBaseline>{};
  ...
  void minorEnableForTypePages({bool withDefaults = true}) {
    ...
    debugPrint(
        '🔔 [Minor] MinorPlateState enabled (NO-SUBSCRIBE) / withDefaults=$withDefaults');
  ...
  void minorSyncWithAreaState() {
    ...
    debugPrint(
        '🔄 [Minor] syncWithAreaState (NO-SUBSCRIBE) → refresh active types');
  ...
  Future<void> minorRefreshType(PlateType type) async {
    ...
    debugPrint(
        '🔎 [Minor][${_getTypeLabel(type)}] 1회 로드 시작 (area=$area, desc=$descending)');
    ...
    try {
        ...
        for (final removedId in removedIds) {
          try {
            ...
            if (sameArea && isDepartureCompleted && isLockedFeeTrue) {
              ...
              debugPrint(
                  '✅ [Minor] 정산 전이 감지(1회 조회 비교): id=$removedId, plate=${fresh.plateNumber}');
            }
          } catch (e) {
      ...
      for (final p in results) {
        final minorizedSelectedBy = p.isSelected
      ...
      debugPrint(
          '✅ [Minor][${_getTypeLabel(type)}] 1회 로드 완료: ${results.length}개');
    } catch (e) {
  ...
  List<PlateModel> minorGetPlatesByCollection(PlateType collection,
  ...
  Future<void> minorUpdatePlateLocally(
