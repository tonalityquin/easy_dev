...
class ModifyParkingWorkspace extends StatefulWidget {
  const ModifyParkingWorkspace({
    super.key,
  ...
  @override
  State<ModifyParkingWorkspace> createState() => _ModifyParkingWorkspaceState();
}

class _ModifyParkingWorkspaceState extends State<ModifyParkingWorkspace> {
  ...
  void _selectDraft(String fullName) {
    _debug('parking_slot=draft location=$fullName');
    widget.onDraftChanged?.call(fullName);
  }

  void _applyDraft() {
    final value = widget.draftLocation?.trim() ?? '';
    if (value.isEmpty) return;
    _debug('parking_slot=applied location=$value');
  ...
  Widget _buildPickerContent(BuildContext context) {
    ...
    return Consumer<LocationState>(
      builder: (context, locationState, _) {
        ...
        final animateCrop = !reduceMotion;
  ...
  Widget build(BuildContext context) {
    ...
    final reduceMotion =
        MediaQuery.maybeOf(context)?.disableAnimations ?? false;
    final draft = widget.draftLocation?.trim() ?? '';
    return Material(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    widget.currentLocation.trim().isEmpty
                        ? '현재 주차 구역이 선택되지 않았습니다.'
                        : '현재 ${widget.currentLocation.trim()}',
                    maxLines: 1,
                          ...
                          children: [
                            Expanded(
                              child: CommonButton(
                                ...
                                expand: true,
                                onPressed: () => widget.onDraftChanged?.call(null),
                              ),
                            ),
                            const SizedBox(width: 8),
                            Expanded(
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              _SectionHeader(
                title: '부모 주차 구역',
                subtitle: '상위 구역 ${topLevels.length}개 · 부모 → 자식 → 빈 자리 선택',
              ),
              const SizedBox(height: 8),
              ...topLevels.map((loc) {
                final grid = loc.parkingGrid;
                final hasGrid = grid != null && grid.rows > 0 && grid.cols > 0;
                final parentKey = _ModifyParkingWorkspaceState._nameKey(
                  loc.locationName,
                );
                final childCount = childDocs.where((child) {
                  return _ModifyParkingWorkspaceState._nameKey(
                        child.parent ?? '',
                      ) ==
                      parentKey;
                }).length;
...
class _InteractiveParkingGridPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final gateCount = grid.entranceRects.length + grid.exitRects.length;
