...
class _ParkingViewMemCache {
  ...
  static Future<List<_ViewRow>> fetch({
    required PlateRepository repository,
    ...
    final locations = await repository.fetchViewLocations(
...
class ModifyLocationBottomSheet extends StatefulWidget {
  final TextEditingController locationController;
  final Function(String) onLocationSelected;

  const ModifyLocationBottomSheet({
    super.key,
  ...
  @override
  State<ModifyLocationBottomSheet> createState() =>
      _ModifyLocationBottomSheetState();
}

class _ModifyLocationBottomSheetState extends State<ModifyLocationBottomSheet> {
...
class _ParentChildViewportSlotFlowState
  ...
  void _backToChildPicker() {
    ...
    _refreshParentOverlay(forceRefresh: false);
  }
    ...
    try {
      final results = await Future.wait(<Future<List<_ViewRow>>>[
        _ParkingViewMemCache.fetch(
          repository: context.read<PlateRepository>(),
