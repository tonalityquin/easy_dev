...
class _MonthlyParkingManagementState extends State<MonthlyParkingManagement> {
  ...
  Future<void> _deleteDoc(String docId) async {
    ...
    try {
      await context.read<PlateRepository>().deleteMonthlyPlateStatus(
            documentId: docId,
          );
      if (!mounted) return;
      ...
      builder: (ctx) {
        return _MonthlyPlateDetailSheet(
          ...
          onEdit: () async {
            ...
            await _openEditSheet(record: record);
          },
          onPay: () async {
            Navigator.of(ctx).pop();
            await _openEditSheet(record: record);
    ...
    return PreferredSize(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border(
            bottom:
                BorderSide(color: cs.outlineVariant.withOpacity(.45), width: 1),
          ),
        ),
        child: Row(
          children: [
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  enabledBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(12),
                    borderSide:
                        BorderSide(color: cs.outlineVariant.withOpacity(.45)),
                  ),
              ...
              Container(
                ...
                child: DropdownButton<_SortMode>(
                  ...
                  icon: Icon(Icons.swap_vert,
                      size: 18, color: cs.onSurface.withOpacity(.65)),
                  borderRadius: BorderRadius.circular(12),
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                chip(
                    label: '전체',
                    value: _ListFilter.all,
                    icon: Icons.all_inbox_outlined),
                const SizedBox(width: 8),
                chip(
                    label: '만료 임박',
                    value: _ListFilter.expiringSoon,
                    icon: Icons.timer_outlined),
                const SizedBox(width: 8),
                chip(
                    label: '만료',
                    value: _ListFilter.expired,
                    icon: Icons.warning_amber_rounded),
                const SizedBox(width: 8),
                chip(
                    label: '메모',
                    value: _ListFilter.hasMemo,
  ...
  List<_MonthlyPlateVM> _buildFilteredSorted(List<PlateStatusRecord> records) {
    final items = records.map((record) {
      final data = record.toMap();
      final docId = record.docId ?? '';
      final plateNumber = docId.split('_').first;
      final countType = (record.countType ?? '').toString();
    ...
    var filtered = items.where((it) {
      ...
      return it.plateNumber.toLowerCase().contains(q) ||
          it.countType.toLowerCase().contains(q);
    }).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<PlateStatusRecord>>(
        stream: context.read<PlateRepository>().watchMonthlyPlateStatuses(
              area: currentArea,
            ),
        builder: (context, snapshot) {
          ...
          final items = _buildFilteredSorted(records);

          return Column(
            children: [
              _buildToolbar(cs, records.length, items.length),
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final amount = ((data['regularAmount'] ?? 0) as num).toInt();
                    ...
                    return _MonthlyPlateCompactCard(
                      ...
                      onTap: () async {
                        await _openDetailSheet(
                          record: PlateStatusRecord.fromMap(data, docId: it.docId),
                          won: won,
                        );
                      },
                    );
...
class _MonthlyPlateCompactCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color border =
        selected ? cs.primary : cs.outlineVariant.withOpacity(.45);
                  ...
                  child: ListView(
                    ...
                    children: [
                      _detailCard(
                        ...
                        children: [
                          _kv(context, '주차 타입',
                              regularType.isEmpty ? '-' : regularType),
          ...
          Row(
            children: [
              Icon(Icons.calendar_today,
                  size: 16, color: cs.onSurface.withOpacity(.60)),
              const SizedBox(width: 6),
              Expanded(
                child: Text(
                  paidAt,
