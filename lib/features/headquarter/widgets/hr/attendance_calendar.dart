...
class AttendanceCalendar extends StatefulWidget {
  const AttendanceCalendar({
    super.key,
    this.presentation = AttendanceCalendarPresentation.page,
  });

  final AttendanceCalendarPresentation presentation;
  ...
  static Future<T?> showAsLeftSideDock<T>(
    ...
    debugPrint(
      '[AttendanceCalendar] side_dock_push_request side=left motion=operations_210_190 translate=-22_to_0 opacity=0.90_to_1 reduceMotion=$reduceMotion',
    );
    try {
      return await showOperationsLeftSideDock<T>(
        context: context,
        barrierLabel: '출·퇴근 관리',
        useRootNavigator: useRootNavigator,
        maxWidth: 360,
        widthFactor: 0.92,
        barrierDismissible: true,
        builder: (_) => const AttendanceCalendar(
          presentation: AttendanceCalendarPresentation.leftSideDock,
        ),
      );
    } finally {
      debugPrint('[AttendanceCalendar] side_dock_closed side=left');
    }
  }
...
class _AttendanceCalendarState extends State<AttendanceCalendar> {
  ...
  bool get _useCommonUi =>
      widget.presentation == AttendanceCalendarPresentation.leftSideDock;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (presetUser != null) {
        unawaited(_loadAttendanceTimes(presetUser, reason: 'preset'));
      }
    });
  }
  ...
  void _recordDebug(String message) {
    final line = '[AttendanceCalendar] $message';
  ...
  Future<void> _showDeveloperStatus() async {
    ...
    final division = user != null && user.divisions.isNotEmpty
        ...
        : '';
    final complete = _completeDayCount;
    ...
    final trace = await DeveloperOperationTrace.start(
      context: context,
      title: '출·퇴근 관리 상태',
      initialMessage: '출·퇴근 관리 상태를 수집하고 있습니다.',
      useCommonUi: true,
    ...
    trace.log(
      'clockIn=${_clockInMap.length}, clockOut=${_clockOutMap.length}, loadedIn=${_loadedClockInMap.length}, loadedOut=${_loadedClockOutMap.length}, complete=$complete, partial=$partial, dirty=$dirty, deleteIn=${_pendingDeleteInDates.length}, deleteOut=${_pendingDeleteOutDates.length}',
      progress: 0.34,
    );
    trace.log(
      'cacheIn=${_inCache.length}, cacheOut=${_outCache.length}, cacheLoadedIn=${_inLoadedCache.length}, cacheLoadedOut=${_outLoadedCache.length}, loading=$_isLoadingMonth, monthError=${_monthLoadError ?? '-'}',
      progress: 0.46,
    );
    trace.log(
    ...
    if (snapshot.isEmpty) {
      trace.log('기록된 출·퇴근 관리 로그가 없습니다.', progress: 0.88);
    } else {
    ...
    await trace.succeed('출·퇴근 관리 상태 수집이 완료되었습니다.');
  }
  ...
  int get _completeDayCount {
    final days = <int>{..._clockInMap.keys, ..._clockOutMap.keys};
    return days.where((day) {
      final hasIn = (_clockInMap[day] ?? '').isNotEmpty;
      final hasOut = (_clockOutMap[day] ?? '').isNotEmpty;
      return hasIn && hasOut;
    }).length;
  }

  int get _partialDayCount {
    final days = <int>{..._clockInMap.keys, ..._clockOutMap.keys};
    return days.where((day) {
      final hasIn = (_clockInMap[day] ?? '').isNotEmpty;
      final hasOut = (_clockOutMap[day] ?? '').isNotEmpty;
  ...
  int get _dirtyDayCount {
    final days = <int>{
      ..._clockInMap.keys,
      ..._clockOutMap.keys,
      ..._loadedClockInMap.keys,
      ..._loadedClockOutMap.keys,
    };
    var count = 0;
    for (final day in days) {
      if ((_clockInMap[day] ?? '') != (_loadedClockInMap[day] ?? '') ||
  ...
  bool _isDirtyDay(int day) {
    return (_clockInMap[day] ?? '') != (_loadedClockInMap[day] ?? '') ||
        (_clockOutMap[day] ?? '') != (_loadedClockOutMap[day] ?? '');
  }
  ...
  Future<void> _selectUser(
    ...
    _userInputFocus.unfocus();
    await _loadAttendanceTimes(user, reason: 'user_select');
  }
  ...
  Future<void> _loadAttendanceTimes(
    UserModel user, {
    ...
    if (_inCache.containsKey(cacheKey) && _outCache.containsKey(cacheKey)) {
      final inMap = <int, String>{..._inCache[cacheKey]!};
      final outMap = <int, String>{..._outCache[cacheKey]!};
      final inLoaded = <int, String>{...(_inLoadedCache[cacheKey] ?? inMap)};
      final outLoaded = <int, String>{...(_outLoadedCache[cacheKey] ?? outMap)};
      if (!mounted) return;
      setState(() {
        _clockInMap = inMap;
        _clockOutMap = outMap;
      ...
      _recordDebug(
        'month_load_cache_hit in=${inMap.length} out=${outMap.length}',
      );
      return;
    }
    ...
    try {
      ...
      setState(() {
        ...
        _outCache[cacheKey] = <int, String>{...outMap};
        _inLoadedCache[cacheKey] = <int, String>{...inMap};
        _outLoadedCache[cacheKey] = <int, String>{...outMap};
        _monthLoadError = null;
      });
      _recordDebug(
        'month_load_remote_complete in=${inMap.length} out=${outMap.length}',
      );
    } catch (error, stackTrace) {
      if (!mounted) return;
  ...
  Future<bool> _persistAllChangesToFirestore() async {
    ...
    _recordDebug(
      'save_persist_start inPayload=${inPayload.length} outPayload=${outPayload.length} deleteIn=${_pendingDeleteInDates.length} deleteOut=${_pendingDeleteOutDates.length}',
    );
    try {
      ...
      setState(() {
        ...
        _outCache[cacheKey] = <int, String>{..._clockOutMap};
        _inLoadedCache[cacheKey] = <int, String>{..._loadedClockInMap};
        _outLoadedCache[cacheKey] = <int, String>{..._loadedClockOutMap};
      });
  ...
  Future<void> _sendMonthlyExcelMail() async {
    ...
    try {
      ...
      if (GoogleAuthSession.isInvalidTokenError(error)) {
        _recordDebug('mail_send_invalid_google_token');
      }
    } finally {
  ...
  Future<void> _changeMonth(int delta) async {
    ...
    if (_selectedUser != null) {
      await _loadAttendanceTimes(_selectedUser!, reason: 'month_change');
    }
  }
  ...
  Future<void> _retryMonthLoad() async {
    final user = _selectedUser;
    if (user == null) return;
    await _loadAttendanceTimes(user, reason: 'retry');
  }
  ...
  Widget _buildDockHeader(BuildContext context) {
    ...
    return CommonAnimatedReveal(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(Icons.how_to_reg_rounded, color: tokens.info, size: 24),
        ...
        Expanded(
          child: CustomScrollView(
            slivers: [
              ...
              SliverToBoxAdapter(
                child: _buildMonthStatus(context),
              ),
              SliverToBoxAdapter(
                child: CommonAnimatedReveal(
  ...
  Widget _buildLegend(BuildContext context) {
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          _FlatLegendItem(color: _success, label: '완료'),
          _FlatLegendItem(color: _warning, label: '부분'),
  ...
  Widget _buildMonthStatus(BuildContext context) {
    ...
    if (_isLoadingMonth) {
      ...
      child = Row(
        ...
        children: [
          Icon(Icons.check_circle_outline_rounded, size: 17, color: _success),
          const SizedBox(width: 7),
          Text(
            '완료 $_completeDayCount · 부분 $_partialDayCount',
            style: text.bodySmall?.copyWith(
              color: tokens.textSecondary,
    ...
    return Padding(
      ...
      child: TableCalendar(
        ...
        onPageChanged: (focusedDay) async {
          ...
          if (_selectedUser != null) {
            await _loadAttendanceTimes(_selectedUser!, reason: 'calendar_page');
          }
        },
  ...
  Widget _buildCell(BuildContext context, DateTime day, DateTime focusedDay) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final timeFs = (baseSide * 0.28).clamp(10.0, 15.0);
        final smallFs = (baseSide * 0.24).clamp(9.0, 13.0);
        final vGap = (baseSide * 0.07).clamp(1.0, 5.0);

        Text timeText(String value, {required bool strong}) {
          return Text(
            value,
            maxLines: 1,
                      ...
                      if (hasIn || hasOut) ...[
                        AnimatedSwitcher(
                          duration: reduceMotion
                              ...
                              : CommonUiMotion.selection,
                          child: KeyedSubtree(
                            key: ValueKey<String>('in_$inTime'),
                            child: timeText(hasIn ? inTime : '—', strong: hasIn),
                          ),
                        ),
                        ...
                        AnimatedSwitcher(
                          duration: reduceMotion
                              ...
                              : CommonUiMotion.selection,
                          child: KeyedSubtree(
                            key: ValueKey<String>('out_$outTime'),
                            child:
                                timeText(hasOut ? outTime : '—', strong: hasOut),
                          ),
                        ),
                      ] else
                        Text(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.presentation == AttendanceCalendarPresentation.page) {
      return Scaffold(
