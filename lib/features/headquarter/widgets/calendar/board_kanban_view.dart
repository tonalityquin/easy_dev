  ...
  Widget build(BuildContext context) {
    ...
    final pages = <_BoardPageData>[
      _BoardPageData(
          ...
          events: buckets[BoardBucket.today] ?? <gcal.Event>[]),
      _BoardPageData(
          title: '이번주',
          bucket: BoardBucket.thisWeek,
          events: buckets[BoardBucket.thisWeek] ?? <gcal.Event>[]),
      _BoardPageData(
          title: '나중에',
          bucket: BoardBucket.later,
          events: buckets[BoardBucket.later] ?? <gcal.Event>[]),
      _BoardPageData(
          title: '완료',
          bucket: BoardBucket.done,
          events: buckets[BoardBucket.done] ?? <gcal.Event>[]),
    ];
...
class _BoardPageData {
  _BoardPageData(
      {required this.title, required this.bucket, required this.events});
...
class _TopTabs extends StatelessWidget {
  const _TopTabs(
      {required this.index, required this.pages, required this.onTap});
              ...
              return _EventCard(
                ...
                onToggleDone: () async {
                  await _safeToggleProgress(
                      context, e, progressOf(e) != 100, onToggleProgress);
                },
              );
            },
          ),
...
Future<void> _safeToggleProgress(
  ...
  gcal.Event e,
  bool nextDone,
  Future<void> Function(BuildContext context, gcal.Event e, bool done)
      onToggleProgress,
) async {
  try {
