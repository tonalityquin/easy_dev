...
class TripleHomeDashBoardController {
  ...
  Future<SheetUploadResult> _recordLeaveTime(BuildContext context) async {
    try {
      ...
      final SheetUploadResult result = await TripleClockOutSave.uploadLeaveJson(
        context: context,
        data: leaveData,
      );
      return result;
  ...
  Future<BreakRecordResult> recordBreakTime(BuildContext context) async {
    try {
      ...
      final SheetUploadResult uploadResult = await TripleBreakSave.uploadBreakJson(
        context: context,
        data: breakJson,
      );
      ...
      if (!uploadResult.success) {
        debugPrint(
          '[triple_home_dash_board_controller] break_record_failed message=${uploadResult.message}',
        );
      ...
      debugPrint(
        '[triple_home_dash_board_controller] break_record_complete at=${now.toIso8601String()}',
      );
      ...
      debugPrint(
        '[triple_home_dash_board_controller] break_record_error error=$error',
      );
