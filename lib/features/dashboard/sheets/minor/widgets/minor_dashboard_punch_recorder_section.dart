...
class MinorDashboardPunchRecorderSection extends StatefulWidget {
  ...
  @override
  State<MinorDashboardPunchRecorderSection> createState() => _MinorDashboardPunchRecorderSectionState();
}
...
class _MinorDashboardPunchRecorderSectionState extends State<MinorDashboardPunchRecorderSection> {
  ...
  void _debug(String message) {
    debugPrint('[MinorPunchRecorder] $message');
  }
  ...
  Future<void> _punch(AttBrkModeType type) async {
    ...
    _debug(
      'punch_start type=${type.code} mode=minor repunch=$repunch dateTime=${targetDateTime.toIso8601String()}',
    );
    try {
      final result = type == AttBrkModeType.breakTime
          ? await CommonAttendanceService.recordBreak(
              context,
              source: 'minor_punch_recorder',
              modeKey: 'minor',
          ...
          : repunch
              ? await CommonAttendanceService.replaceClockOut(
                  context,
                  source: 'minor_punch_recorder_repunch',
                  modeKey: 'minor',
                  recordedAt: targetDateTime,
                )
      ...
      _debug('punch_complete type=${type.code} mode=minor repunch=$repunch');
      if (type == AttBrkModeType.workOut && mounted) {
        await _exitAppAfterClockOut(context);
      }
    } catch (error, stackTrace) {
      _debug(
        'punch_failure type=${type.code} mode=minor repunch=$repunch error=$error stack=$stackTrace',
      );
      rethrow;
    } finally {
  ...
  Future<void> _showDeveloperStatus() async {
    ...
    trace.log('component=MinorPunchRecorder', progress: .16);
    trace.log(
