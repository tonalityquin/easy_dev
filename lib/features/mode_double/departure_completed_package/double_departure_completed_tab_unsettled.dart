...
class DoubleDepartureCompletedUnsettledTab extends StatefulWidget {
  ...
  @override
  State<DoubleDepartureCompletedUnsettledTab> createState() =>
      _DoubleDepartureCompletedUnsettledTabState();
}

class _DoubleDepartureCompletedUnsettledTabState
    extends State<DoubleDepartureCompletedUnsettledTab> {
  ...
  Future<void> _refreshUnsettled() async {
    ...
    if (area.isEmpty) {
      debugPrint('현재 지역(area)이 설정되지 않아 갱신할 수 없습니다.');
      return;
    }
    ...
    final lastAt =
    plateState.doubleLastRefreshAtOf(PlateType.departureCompleted);
    final sourceLabel =
    plateState.doubleLastRefreshSourceLabelOf(PlateType.departureCompleted);
  ...
  List<Widget> _buildGroupedList(
      List<PlateModel> plates, Map<DateTime, int> dateCounts) {
...
class _UnsettledPlateCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final location = plate.location.trim().isEmpty ? '미지정' : plate.location.trim();
