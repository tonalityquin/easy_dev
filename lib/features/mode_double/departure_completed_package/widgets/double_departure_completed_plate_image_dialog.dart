...
List<Widget> _buildInfoLines(BuildContext context, _PlateImageMeta meta) {
  final cs = Theme.of(context).colorScheme;
  final style = TextStyle(fontSize: 14, color: cs.onSurface);
  final mutedStyle = TextStyle(fontSize: 14, color: cs.onSurfaceVariant);
  final dateTime = meta.dateTimeText;
  final widgets = <Widget>[];

  if (dateTime.isNotEmpty) {
    widgets.add(Text('📅 $dateTime', style: style));
  }
  if (meta.plate.isNotEmpty) {
    widgets.add(Text('🚘 ${meta.plate}', style: style));
  }
