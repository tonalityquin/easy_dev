...
Future<PlateModel?> showDoubleDepartureCompletedStatusBottomSheet({
  ...
  trace.log(
    '[DoubleDepartureSearchStatus] presentation=right_side_dock direction=right_to_left plate=$plateNumber area=$area',
    progress: .16,
  );
...
class _StatusSideDockContentState extends State<_StatusSideDockContent> {
  ...
  Future<void> _handlePrebill() async {
    ...
    try {
      ...
      parkingStatusTraceLog(context,'사전 정산 완료: ₩${result.lockedFee} (${result.paymentMethod})');
    } catch (e) {
      if (!mounted) return;
