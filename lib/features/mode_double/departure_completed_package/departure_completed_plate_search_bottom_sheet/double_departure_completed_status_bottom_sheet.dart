...
Future<PlateModel?> showDoubleDepartureCompletedStatusBottomSheet({
  ...
  trace.log(
    '[DoubleDepartureSearchStatus] presentation=right_side_dock direction=right_to_left plate=${plate.plateNumber} area=$area',
    progress: .16,
  );
...
class _StatusSideDockContentState extends State<_StatusSideDockContent> {
  ...
  Future<bool> _autoPrebillFreeIfNeeded() async {
    ...
    try {
      ...
      _reportDbSafe(
        area: _plate.area,
        action: 'write',
        source: 'departureCompletedStatus.freeAutoPrebill.repo.appendPlateLog',
        n: 1,
      );
  ...
  Future<void> _handlePrebill() async {
    ...
    final request = PlateBillingSideDockRequest(
      plate: plateSnapshot,
      source: 'double_departure_search_status',
      onSubmit: (result) async {
        final now = DateTime.now();
        final settledAt = now.toUtc().millisecondsSinceEpoch ~/ 1000;
        final updatedPlate = plateSnapshot.copyWith(
          isLockedFee: true,
          lockedAtTimeInSeconds: settledAt,
          lockedFeeAmount: result.lockedFee,
          paymentMethod: result.paymentMethod,
        );
        await repo.addOrUpdatePlate(plateSnapshot.id, updatedPlate);
        _reportDbSafe(
        ...
        await repo.appendPlateLog(
          plateId: plateSnapshot.id,
          log: log,
        );
        _reportDbSafe(
          area: plateSnapshot.area,
          action: 'write',
          source: 'departureCompletedStatus.prebill.repo.appendPlateLog',
          n: 1,
        );
        return updatedPlate;
      },
