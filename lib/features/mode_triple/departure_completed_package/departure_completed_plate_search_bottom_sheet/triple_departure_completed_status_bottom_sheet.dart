...
Future<PlateModel?> showTripleDepartureCompletedStatusBottomSheet({
  ...
  trace.log(
    '[TripleDepartureSearchStatus] presentation=right_side_dock direction=right_to_left plate=${plate.plateNumber} area=$area',
    progress: .16,
  );
...
class _StatusSideDockContentState extends State<_StatusSideDockContent> {
  ...
  Future<void> _handlePreBill() async {
    ...
    final repo = context.read<PlateRepository>();
    final plateState = context.read<TriplePlateState>();
    final plateId = plateSnapshot.id.trim().isNotEmpty ? plateSnapshot.id.trim() : '${plateSnapshot.plateNumber}_${plateSnapshot.area}';
    final request = PlateBillingSideDockRequest(
      plate: plateSnapshot,
      source: 'triple_departure_search_status',
