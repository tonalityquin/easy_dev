...
class _DoubleSelectionBaseline {
  ...
  const _DoubleSelectionBaseline(
...
class DoublePlateState extends ChangeNotifier {
  ...
  DateTime? doubleLastRefreshAtOf(PlateType type) =>
      _lastDoubleRefreshAtByType[type];

  String doubleLastRefreshSourceLabelOf(PlateType type) =>
      _lastDoubleRefreshSourceByType[type] ?? '-';
  ...
  final Map<String, _DoubleSelectionBaseline> _baseline =
      <String, _DoubleSelectionBaseline>{};
  ...
  void doubleEnableForTypePages({bool withDefaults = true}) {
    ...
    debugPrint(
        '🔔 [Double] DoublePlateState enabled (NO-SUBSCRIBE) / withDefaults=$withDefaults');
  ...
  void doubleSyncWithAreaState() {
    ...
    debugPrint(
        '🔄 [Double] syncWithAreaState (NO-SUBSCRIBE) → refresh active types');
  ...
  Future<void> doubleRefreshType(PlateType type) async {
    ...
    debugPrint(
        '🔎 [Double][${_getTypeLabel(type)}] 1회 로드 시작 (area=$area, desc=$descending)');
    ...
    try {
        ...
        for (final removedId in removedIds) {
          try {
            ...
            if (sameArea && isDepartureCompleted && isLockedFeeTrue) {
              ...
              debugPrint(
                  '✅ [Double] 정산 전이 감지(1회 조회 비교): id=$removedId, plate=${fresh.plateNumber}');
            }
          } catch (e) {
      ...
      debugPrint(
          '✅ [Double][${_getTypeLabel(type)}] 1회 로드 완료: ${results.length}개');
    } catch (e) {
  ...
  List<PlateModel> doubleGetPlatesByCollection(PlateType collection,
  ...
  Future<void> doubleUpdatePlateLocally(
  ...
  void _onAreaChanged() {
    ...
    debugPrint(
        '🔄 [Double] area changed → refresh active types (NO-SUBSCRIBE)');
