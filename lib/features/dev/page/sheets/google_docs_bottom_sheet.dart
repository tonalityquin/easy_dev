...
class _GoogleDocsDocBottomSheetState extends State<_GoogleDocsDocBottomSheet> {
  ...
  Future<void> _saveDocument() async {
    try {
      ...
      final doc = await api.documents.get(id);
      final endIndex = _getDocumentEndIndex(doc);
      ...
      if (table != null) {
        for (final row in table.tableRows ?? const <gdocs.TableRow>[]) {
          for (final cell in row.tableCells ?? const <gdocs.TableCell>[]) {
            ...
            for (final cse
                in cell.content ?? const <gdocs.StructuralElement>[]) {
              final p = cse.paragraph;
              if (p != null) {
                for (final ce
                    in p.elements ?? const <gdocs.ParagraphElement>[]) {
                  final tr = ce.textRun;
