...
import 'package:flutter/widgets.dart' show LogicalKeySet, Intent;
...
class DevMemo {
  ...
  static const String kDriveFolderName = '00.IdeaNote';
  static const String kDriveLegacyFolderName = 'DevMemo';
  static const String kDriveFileName = '00.IdeaNote.md';
  static const String kDriveLegacyFileName = 'DevMemo.md';
...
class _DevMemoSheetState extends State<_DevMemoSheet> {
  ...
  Map<String, String> _fileIdCache = {};
  ...
  Future<void> _loadInitial() async {
    ...
    _currentFileName = prefs.getString(DevMemo._kDriveSelectedFileName) ??
        DevMemo.kDriveFileName;
  ...
  Widget build(BuildContext context) {
    ...
    final shortcuts = <ShortcutActivator, Intent>{
      LogicalKeySet(
          ...
          const _MdIntent(_MdCmd.inlineCode),
      LogicalKeySet(
              LogicalKeyboardKey.controlRight, LogicalKeyboardKey.backquote):
          const _MdIntent(_MdCmd.inlineCode),
      LogicalKeySet(LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.backquote):
          const _MdIntent(_MdCmd.inlineCode),
      LogicalKeySet(LogicalKeyboardKey.metaRight, LogicalKeyboardKey.backquote):
          const _MdIntent(_MdCmd.inlineCode),
      LogicalKeySet(
          LogicalKeyboardKey.controlLeft,
          LogicalKeyboardKey.shiftLeft,
          LogicalKeyboardKey.keyC): const _MdIntent(_MdCmd.checkbox),
      LogicalKeySet(
          LogicalKeyboardKey.controlLeft,
      ...
      LogicalKeySet(LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.shiftLeft,
          LogicalKeyboardKey.keyC): const _MdIntent(_MdCmd.checkbox),
      LogicalKeySet(LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.shiftRight,
          LogicalKeyboardKey.keyC): const _MdIntent(_MdCmd.checkbox),
      LogicalKeySet(LogicalKeyboardKey.metaRight, LogicalKeyboardKey.shiftLeft,
          LogicalKeyboardKey.keyC): const _MdIntent(_MdCmd.checkbox),
      LogicalKeySet(LogicalKeyboardKey.metaRight, LogicalKeyboardKey.shiftRight,
          LogicalKeyboardKey.keyC): const _MdIntent(_MdCmd.checkbox),
      LogicalKeySet(
          LogicalKeyboardKey.controlLeft,
          LogicalKeyboardKey.shiftLeft,
          LogicalKeyboardKey.keyT): const _MdIntent(_MdCmd.toggleBlock),
      LogicalKeySet(
          LogicalKeyboardKey.controlLeft,
          LogicalKeyboardKey.shiftRight,
          LogicalKeyboardKey.keyT): const _MdIntent(_MdCmd.toggleBlock),
      ...
      LogicalKeySet(LogicalKeyboardKey.controlLeft, LogicalKeyboardKey.keyB):
          const _MdIntent(_MdCmd.bold),
      LogicalKeySet(LogicalKeyboardKey.controlRight, LogicalKeyboardKey.keyB):
          const _MdIntent(_MdCmd.bold),
      LogicalKeySet(LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.keyB):
          const _MdIntent(_MdCmd.bold),
      LogicalKeySet(LogicalKeyboardKey.metaRight, LogicalKeyboardKey.keyB):
          const _MdIntent(_MdCmd.bold),
      LogicalKeySet(LogicalKeyboardKey.controlLeft, LogicalKeyboardKey.keyI):
          const _MdIntent(_MdCmd.italic),
      LogicalKeySet(LogicalKeyboardKey.controlRight, LogicalKeyboardKey.keyI):
          const _MdIntent(_MdCmd.italic),
      LogicalKeySet(LogicalKeyboardKey.metaLeft, LogicalKeyboardKey.keyI):
          const _MdIntent(_MdCmd.italic),
      LogicalKeySet(LogicalKeyboardKey.metaRight, LogicalKeyboardKey.keyI):
          const _MdIntent(_MdCmd.italic),
    };
                  ...
                  child: Column(
                    children: [
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              tooltip: '파일 선택/변경(.md)',
                              onPressed:
                                  _driveBusy ? null : _openChooseFileDialog,
                              icon: const Icon(Icons.drive_file_rename_outline),
                            ),
                            IconButton(
                              tooltip:
                                  _driveBusy ? '불러오는 중...' : 'Drive에서 불러오기',
                            ...
                            IconButton(
                              ...
                              onPressed:
                                  _sending ? null : _sendAsMarkdownByEmail,
                              icon: _sending
                                  ? const SizedBox(
                                      width: 18,
                                      height: 18,
                      ...
                      _MdToolbar(
                        ...
                        onLink: () =>
                            _insertTemplate('[텍스트](https://example.com)'),
                        onImage: () {},
                        onTable: () {},
                      ),
                      Expanded(
  ...
  Future<void> _sendAsMarkdownByEmail() async {
    ...
    try {
      ...
      final subject =
          'DevMemo export (${DateFormat('yyyy-MM-dd').format(now)})';
      final filename =
          'dev_memo_${DateFormat('yyyyMMdd_HHmmss').format(now)}.md';
...
''';
  ...
  void _wrapSelection(String prefix, String suffix, String placeholder) {
    ...
    _replaceRange(start, end, replaced,
        newSelection: hasSelection
            ? TextSelection(
                baseOffset: start, extentOffset: start + replaced.length)
            : TextSelection.collapsed(
                offset: start + prefix.length + middle.length));
  }
  ...
  Future<void> _clearDriveIdCache() async {
    ...
    await prefs.remove(DevMemo._kDriveFileIdMap);
    await prefs.remove(DevMemo._kDriveSelectedFileName);
  }
  ...
  Future<String?> _validateId(
    ...
    try {
      ...
      if (expectFolder && file.mimeType != 'application/vnd.google-apps.folder')
        return null;
      if (!expectFolder &&
          file.mimeType == 'application/vnd.google-apps.folder') return null;
      return file.id;
    } catch (_) {
      return null;
    }
  ...
  Future<List<drive.File>> _listAllMarkdownInFolder(
    ...
    String? pageToken;
    final q =
        "'$folderId' in parents and trashed = false and name contains '.md'";
  ...
  Future<void> _openChooseFileDialog() async {
    ...
    final prefs = DevMemo._prefs ?? await SharedPreferences.getInstance();
    String? folderId = await _validateId(
        api, prefs.getString(DevMemo._kDriveFolderId),
        expectFolder: true);
    ...
    final namesInDrive =
        driveFiles.map((f) => f.name ?? '').where((s) => s.isNotEmpty);
  ...
  Future<void> _driveLoadSelected(String filename, {bool retry = true}) async {
    ...
    try {
      ...
      if (fileId == null) {
        ...
        final r =
            await api.files.list(q: q, $fields: 'files(id,name,modifiedTime)');
        if (r.files != null && r.files!.isNotEmpty) {
          fileId = r.files!.first.id!;
        } else {
          if (filename == DevMemo.kDriveFileName) {
            final qOld =
                "'$folderId' in parents and name = '${DevMemo.kDriveLegacyFileName}' and trashed = false";
            final rOld = await api.files
                .list(q: qOld, $fields: 'files(id,name,modifiedTime)');
      ...
      if (fileId != null) {
        ...
        _docCtrl.text = text;
        _docCtrl.selection =
            TextSelection.collapsed(offset: _docCtrl.text.length);
  ...
  Future<void> _driveSaveSelected(String filename, {bool retry = true}) async {
    ...
    try {
      ...
      final media =
          drive.Media(Stream<List<int>>.fromIterable([bytes]), bytes.length);
  ...
  Future<void> _driveLoad({bool retry = true}) =>
      _driveLoadSelected(_currentFileName, retry: retry);

  Future<void> _driveSave({bool retry = true}) =>
      _driveSaveSelected(_currentFileName, retry: retry);
...
class _MdToolbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          _ttIcon(
              context, Icons.code_rounded, '인라인 코드 (Ctrl/Cmd+`)', onInlineCode),
          _ttIcon(context, Icons.check_box_outlined, '체크박스 (Ctrl/Cmd+Shift+C)',
              onCheckbox),
          _ttIcon(context, Icons.expand_more_rounded,
              '토글 블록 (Ctrl/Cmd+Shift+T)', onToggleBlock),
          _ttIcon(
              context, Icons.format_bold_rounded, '굵게 (Ctrl/Cmd+B)', onBold),
          _ttIcon(context, Icons.format_italic_rounded, '기울임 (Ctrl/Cmd+I)',
              onItalic),
          _chip(context, 'H1', onH1, cs.primary),
          _chip(context, 'H2', onH2, cs.secondary),
          _ttIcon(
              context, Icons.format_list_bulleted_rounded, '불릿 목록', onBullet),
          _ttIcon(
              context, Icons.format_list_numbered_rounded, '번호 목록', onNumbered),
  ...
  Widget _chip(
      BuildContext context, String label, VoidCallback onTap, Color bg) {
    ...
    return InkWell(
      ...
      child: Ink(
        ...
        child: Text(label,
            style: TextStyle(fontWeight: FontWeight.w700, color: on)),
      ),
    );
  }
}
