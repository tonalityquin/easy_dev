...
// ✅ 5초 취소 가능 다이얼로그(기본 프리뷰 카드 사용)
import '../../../../utils/block_dialogs/real_time_duration_dialog.dart';
...
class _Brand {
  ...
  static Color rowAlt(ColorScheme cs) => cs.surfaceContainerLow;
}

/// UI 렌더링 Row VM
/// ✅ primaryTimeField 값과 updatedAt을 분리 보관 + createdAt(정렬용) 유지
class _RowVM {
  final String plateId; // plates docId
  final String plateNumber;
  final String location;

  /// 탭별 primaryTimeField 값
  final DateTime? primaryAt;

  /// view updatedAt
  final DateTime? updatedAt;

  /// 정렬/그룹핑 기준: primaryAt ?? updatedAt
...
/// - 조회/캐시/쿨다운/파싱만 담당
/// ✅ 참고:
///   - plateNumber / plate_number 둘 다 지원
///   - primaryTimeField + updatedAt 분리 파싱
/// ─────────────────────────────────────────────────────────
  ...
  Future<List<_RowVM>> fetchFromServerAndCache(String area) async {
    ...
    if (items is Map) {
      for (final entry in items.entries) {
        ...
        // ✅ plateNumber / plate_number 둘 다 지원
        final plateNumber = (m['plateNumber'] as String?) ??
            (m['plate_number'] as String?) ??
            _fallbackPlateFromDocId(plateDocId);

        final location = _normalizeLocation(m['location'] as String?);

        final primaryAt = _toDate(m[primaryTimeField]);
...
class _MinorParkingCompletedRealTimeTableState
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          // ✅ 수정안 반영: 상단 헤더/닫기(X)/구분선 제거 → 데이터 영역 확장
          Expanded(
            child: TabBarView(
              controller: _tabCtrl,
              ...
              children: [
                _reqGate
                  ...
                  description:
                  _descriptionForMode(_TabMode.parkingRequestsRealtime),
                )
                ...
                _pcGate
                  ...
                  description:
                  _descriptionForMode(_TabMode.parkingCompletedRealtime),
                )
                    : const _RealtimeTabLockedPanel(
...
/// ✅ [리팩터링] VIEW "텍스트 프리뷰" 미사용(다이얼로그는 기본 프리뷰 카드만 사용)
/// ─────────────────────────────────────────────────────────
...
class _UnifiedTableTabState extends State<_UnifiedTableTab>
  ...
  Future<void> _openHybridDetailPopup(_RowVM r) async {
    ...
    try {
      ...
      // ✅ VIEW "텍스트 프리뷰" 미사용:
      //  - body 전달하지 않음
      //  - plateNumber/location/occurredAt 전달 → 다이얼로그 기본 프리뷰 카드 렌더링
      ...
      plate.isSelected ? (plate.selectedBy ?? '') : plate.userName;
  ...
  Widget _buildCooldownChip(ColorScheme cs, TextTheme text) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(blocked ? Icons.timer_outlined : Icons.check_circle_outline,
              size: 16, color: fg),
          const SizedBox(width: 6),
          Text(
            label,
                        ...
                        children: [
                          Expanded(
                            ...
                            child: Text(
                              noText,
                              style:
                              monoStyle.copyWith(fontWeight: FontWeight.w900),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                            ),
                          ),
  ...
  List<_ZoneGroupVM> _buildZoneGroupsFromCurrentData() {
    ...
    // ✅ 참고 코드 스타일 정렬(standalone도 잔여 우선 + 이름)
    ...
    // ✅ composite 그룹도 참고 코드처럼 child 정렬 추가
