...
class MinorParkingCompletedRepository {
  final ParkingCompletedDb _dbProvider;
  MinorParkingCompletedRepository({ParkingCompletedDb? dbProvider})
      : _dbProvider = dbProvider ?? ParkingCompletedDb.instance;

  Future<int> insert(MinorParkingCompletedRecord record) async {
  ...
  Future<List<MinorParkingCompletedRecord>> listAll({
    int limit = 500,
    ...
    return rows.map((m) => MinorParkingCompletedRecord.fromMap(m)).toList();
  }
