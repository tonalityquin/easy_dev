...
import 'commute_inside_package/lite_commute_inside_controller.dart';
import 'commute_inside_package/sections/lite_commute_inside_work_button_section.dart';
import 'commute_inside_package/sections/lite_commute_inside_user_info_card_section.dart';
import 'commute_inside_package/sections/lite_commute_inside_header_widget_section.dart';

class LiteCommuteInsideScreen extends StatefulWidget {
  const LiteCommuteInsideScreen({super.key});

  @override
  State<LiteCommuteInsideScreen> createState() => _LiteCommuteInsideScreenState();
}

class _LiteCommuteInsideScreenState extends State<LiteCommuteInsideScreen> {
  final controller = LiteCommuteInsideController();
  String? kakaoUrl;
  ...
  Widget _buildScreenTag(BuildContext context) {
    ...
    return Positioned(
      ...
      child: IgnorePointer(
        child: Semantics(
          label: 'screen_tag: lite commute screen',
          child: Text('lite commute screen', style: style),
        ),
      ),
    );
  }
                  ...
                  SingleChildScrollView(
                    child: Center(
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            const LiteCommuteInsideHeaderWidgetSection(),
                            const LiteCommuteInsideUserInfoCardSection(),
                            const SizedBox(height: 12),

                            // 🔹 업무 보고 버튼 제거 → 근무 버튼만 전체 폭으로 배치
                            SizedBox(
                              width: double.infinity,
                              child: LiteCommuteInsideWorkButtonSection(
