...
class LiteCommuteInsideController {
  ...
  Future<LiteCommuteDestination> _decideDestination(
    ...
    if (!userState.isWorking) return LiteCommuteDestination.none;
    if (!context.mounted) return LiteCommuteDestination.none;
    ...
    try {
      ...
      return isHq ? LiteCommuteDestination.headquarter : LiteCommuteDestination.type;
    } catch (e) {
      debugPrint('❌ _decideDestination 실패: $e');
      return LiteCommuteDestination.none;
    }
  }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      switch (dest) {
        case LiteCommuteDestination.headquarter:
          Navigator.pushReplacementNamed(context, AppRoutes.liteHeadquarterPage);
          break;
        case LiteCommuteDestination.type:
          Navigator.pushReplacementNamed(context, AppRoutes.liteTypePage);
          break;
  ...
  Future<dynamic> _uploadAttendanceSilently(BuildContext context) async {
    ...
    final result = await LiteCommuteInsideClockInLogUploader.uploadAttendanceJson(
      context: context,
      data: {
