...
///   - 정산은 "번호판 검색"으로 진행 안내
///   - 마지막 갱신 시간 표시 + 색상 정책
///     - 날짜가 오늘과 다르면 error 계열
///     - 오늘이지만 1시간 이상 경과하면 tertiary 계열
/// - 하단: "데이터 갱신" + "번호판 검색"
/// - Pull-to-refresh 지원
class TripleDepartureCompletedUnsettledTab extends StatefulWidget {
  const TripleDepartureCompletedUnsettledTab({
    super.key,
    required this.firestorePlates, // 시그니처 호환(실제 렌더링은 TriplePlateState 최신 데이터 사용)
...
class _TripleDepartureCompletedUnsettledTabState
  ...
  /// ✅ 데이터 갱신 로직: TriplePlateState에 출차완료 1회 재조회 요청
  Future<void> _refreshUnsettled() async {
    ...
    final lastAt = plateState.tripleLastRefreshAtOf(PlateType.departureCompleted);
    final sourceLabel =
...
///
/// 색상 정책(브랜드 팔레트 기반):
/// - lastRefreshAt == null: error
/// - 오늘 날짜와 다름: error
/// - 오늘이지만 1시간 이상 경과: tertiary
...
class _DateDivider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(
          child: Divider(height: 1, color: cs.outlineVariant.withOpacity(0.85)),
        ),
        Padding(
