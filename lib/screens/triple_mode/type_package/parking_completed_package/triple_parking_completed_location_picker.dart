...
/// ✅ (임베드 버전) 기존 LocationPicker 자리 대체: “실시간(view) 테이블(2탭)”
/// - 트리플 모드: 입차 요청 없음 → (입차 완료 / 출차 요청)만 제공
/// - 내부 Scaffold 제거 → 외부 ControlButtons가 계속 보이고, body 영역까지만 자연스럽게 채움.
class TripleParkingCompletedLocationPicker extends StatefulWidget {
  final VoidCallback? onClose;

  const TripleParkingCompletedLocationPicker({
    super.key,
    this.onClose,
  });

  @override
  State<TripleParkingCompletedLocationPicker> createState() =>
      _TripleParkingCompletedLocationPickerState();
}

class _TripleParkingCompletedLocationPickerState
    extends State<TripleParkingCompletedLocationPicker>
