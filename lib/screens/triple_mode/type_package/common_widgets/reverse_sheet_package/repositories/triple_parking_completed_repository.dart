...
import '../data/normal_pc_db.dart';
import '../models/triple_parking_completed_record.dart';

class TripleParkingCompletedRepository {
  final ParkingCompletedDb _dbProvider;
  TripleParkingCompletedRepository({ParkingCompletedDb? dbProvider})
      : _dbProvider = dbProvider ?? ParkingCompletedDb.instance;

  Future<int> insert(TripleParkingCompletedRecord record) async {
  ...
  Future<List<TripleParkingCompletedRecord>> listAll({
    int limit = 500,
    ...
    return rows.map((m) => TripleParkingCompletedRecord.fromMap(m)).toList();
  }
