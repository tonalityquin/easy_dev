...
class _ZoneVM {
  final String fullName; // 드롭다운/집계(rawCounts) 키로 쓰는 "전체 표시명"
  final String group; // composite parent(부모명). 단독은 ''.
  final String displayName; // 화면 표시용(단독=fullName, 복합 child=leaf)
  final String leaf;
  final int capacity;
  final int current;
  final int? remaining;
...
class _ZoneGroupVM {
  final String group; // group == '' 인 경우: 단독 리스트(헤더 없이 출력)
...
class _RowVM {
  ...
  /// ✅ VIEW의 primaryTimeField(= parkingCompletedAt)
  final DateTime? parkingCompletedAt;

  /// ✅ VIEW의 updatedAt
  final DateTime? updatedAt;

  /// 정렬/그룹핑 용(기존 로직 유지): parkingCompletedAt ?? updatedAt
  ...
  Future<List<_RowVM>> fetchFromServerAndCache(String area) async {
    ...
    if (items is Map) {
      for (final entry in items.entries) {
        ...
        // ✅ timestamp 분리 보관
        final parkingCompletedAt = _toDate(m[primaryTimeField]);
        final updatedAt = _toDate(m['updatedAt']);
        final createdAt = parkingCompletedAt ?? updatedAt;
...
class _UnifiedTableBodyState extends State<_UnifiedTableBody>
  ...
  Widget _buildCooldownChip(ColorScheme cs, TextTheme text) {
    ...
    final bg = blocked ? cs.errorContainer : cs.tertiaryContainer;
    final fg = blocked ? cs.onErrorContainer : cs.onTertiaryContainer;
    final icon = blocked ? Icons.timer_outlined : Icons.check_circle_outline;
  ...
  // (이하 _buildZoneAccordion / 다이얼로그 / 상세 팝업 등은 원본 구조 유지)
  // ─────────────────────────────────────────────────────────
  ...
  Widget _buildZoneAccordion(ColorScheme cs, TextTheme text) {
    ...
    for (final g in groups) {
      ...
      final groupRemainColor = (g.totalRemaining != null && g.totalRemaining! <= 0)
          ? cs.error
          : cs.tertiary;
  ...
  Future<void> _openHybridDetailPopup(_RowVM r) async {
    ...
    try {
      ...
      // ✅ duration dialog에는 기본 프리뷰 카드(번호판/구역/시간)를 출력
      final proceed = await showRealTimeDurationBlockingDialog(
        context,
        message: '', // 메시지 대신 프리뷰 카드 사용
        duration: const Duration(seconds: 5),
        plateNumber: r.plateNumber,
        location: r.location,
        occurredAt: r.createdAt, // 또는 r.parkingCompletedAt
      );
