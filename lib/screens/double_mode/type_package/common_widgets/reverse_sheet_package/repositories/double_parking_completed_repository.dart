...
import '../data/double_pc_db.dart';
import '../models/double_parking_completed_record.dart';

class DoubleParkingCompletedRepository {
  final ParkingCompletedDb _dbProvider;
  DoubleParkingCompletedRepository({ParkingCompletedDb? dbProvider})
      : _dbProvider = dbProvider ?? ParkingCompletedDb.instance;

  Future<int> insert(DoubleParkingCompletedRecord record) async {
  ...
  Future<List<DoubleParkingCompletedRecord>> listAll({
    int limit = 500,
    ...
    return rows.map((m) => DoubleParkingCompletedRecord.fromMap(m)).toList();
  }
