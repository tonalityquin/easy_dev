...
class SimpleEndWorkReportService {
  ...
  Future<SimpleEndWorkReportResult> submitEndReport({
    ...
    try {
      dev.log(
        '[END] save report to Firestore (monthly sharding + meta upsert in one batch)...',
        name: 'SimpleEndWorkReportService',
      );
      ...
      // end_work_reports/area_<area>/months/<yyyyMM>
      // end_work_reports/area_<area>/months/<yyyyMM>/reports/<yyyy-MM-dd>
      final areaDocRef = _firestore.collection('end_work_reports').doc('area_$area');
      final monthDocRef = areaDocRef.collection('months').doc(monthKey);
      final dailyReportRef = monthDocRef.collection('reports').doc(dateStr);

      // history는 "일별 report 문서"의 필드로 유지(FieldValue.arrayUnion)
      ...
      // 일별 report 문서 payload (신규 문서 기반)
      final Map<String, dynamic> dailyPayload = <String, dynamic>{
        'division': division,
        'area': area,
        'date': dateStr,
        'monthKey': monthKey,
      ...
      final Map<String, dynamic> monthMetaPayload = <String, dynamic>{
        'division': division,
        'area': area,
        'monthKey': monthKey,
        'updatedAt': now.toIso8601String(),
      ...
      batch.set(monthDocRef, monthMetaPayload, SetOptions(merge: true));
      batch.set(dailyReportRef, dailyPayload, SetOptions(merge: true));

      await batch.commit();
    } catch (e, st) {
      firestoreSaveOk = false;
      dev.log(
        '[END] Firestore save failed (monthly sharding)',
        name: 'SimpleEndWorkReportService',
        error: e,
        stackTrace: st,
      );
