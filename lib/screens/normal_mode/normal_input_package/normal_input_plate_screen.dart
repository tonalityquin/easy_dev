...
import 'sections/normal_input_bill_section.dart';
import 'sections/normal_input_location_section.dart';
import 'sections/normal_input_photo_section.dart';
import 'sections/normal_input_plate_section.dart';
import 'sections/normal_input_bottom_action_section.dart';
import 'sections/normal_input_custom_status_section.dart';

import 'keypad/num_keypad.dart';
import 'keypad/kor_keypad.dart';
import 'widgets/normal_input_bottom_navigation.dart';

import 'normal_live_ocr_page.dart';
...
class NormalInputPlateScreen extends StatefulWidget {
  const NormalInputPlateScreen({super.key});

  @override
  State<NormalInputPlateScreen> createState() => _NormalInputPlateScreenState();
}

class _NormalInputPlateScreenState extends State<NormalInputPlateScreen> {
  final controller = NormalInputPlateController();
  ...
  static const String screenTag = 'normal plate input';
  ...
  // - Normal vs Service 차이는 source 프리픽스로 구분(Normal. / svc.)
  // ─────────────────────────────
  static const bool _usageUseSourceOnlyKey = true;
  static const int _usageSourceShardCount = 10;

  /// ✅ (변경) Normal 화면에서 번호판 입력 완료 시 plate_status 조회 → UI 프리필
  static const String _usageSrcNormalPlateStatusLookupOnComplete =
      'normal.plate_status.lookup.on_complete';

  // 아래 2개는 기존 문자열을 유지(대시보드/집계 호환성)
  static const String _usageSrcMonthlyLookup =
      'NormalInputPlateScreen._fetchMonthlyPlateStatus/monthly_plate_status.lookup';
  static const String _usageSrcMonthlyUpdate =
      'NormalInputPlateScreen._applyMonthlyMemoAndStatusOnly/monthly_plate_status.doc.update';
  ...
  void _requestExit({bool defer = false}) {
    ...
    void doPop() {
      ...
      try {
        ...
        debugPrint('[NormalInputPlateScreen] pop failed: $e');
      }
    }
  ...
  void initState() {
    ...
    // ⬇️ 첫 빌드 직후 한 번만 자동으로 NormalLiveOcrPage 열기
    WidgetsBinding.instance.addPostFrameCallback((_) async {
  ...
  Future<Map<String, dynamic>?> _fetchPlateStatus(
    ...
    try {
      ...
      await _reportUsage(
        ...
        action: 'read',
        n: 1,
        source: _usageSrcNormalPlateStatusLookupOnComplete,
      );
    }
  }
  ...
  Future<void> _openLiveScanner() async {
    final plate = await Navigator.of(context).push<String>(
      MaterialPageRoute(builder: (_) => const NormalLiveOcrPage()),
    );
  ...
  Widget _buildBottomBar() {
    final actionButton = NormalInputBottomActionSection(
      controller: controller,
  ...
  Widget _buildDockPagedBody({required bool canSwipe}) {
    final Widget page = (_dockPageIndex == _dockPageBill)
      ...
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        NormalInputCustomStatusSection(
          controller: controller,
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        NormalInputLocationSection(
                            locationController: controller.locationController),
                        const SizedBox(height: 16),
                        NormalInputPhotoSection(
...
/// - Normal 테마 색상(AppCardPalette.NormalBase/NormalDark/NormalLight) 사용
class _PlateStatusLoadedDialog extends StatelessWidget {
  ...
  Widget _infoCard({
    ...
    final base = palette.normalBase;
    final dark = palette.normalDark;
    final light = palette.normalLight;
...
class _SheetHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) {
    ...
    final bool billSelected =
        currentPageIndex == _NormalInputPlateScreenState._dockPageBill;
    final bool memoSelected =
        currentPageIndex == _NormalInputPlateScreenState._dockPageMemo;
