...
import 'normal_departure_completed_package/normal_departure_completed_tab_settled.dart';
import 'normal_departure_completed_package/normal_departure_completed_tab_unsettled.dart';
import 'normal_departure_completed_package/widgets/normal_departure_completed_selected_date_bar.dart';

class NormalDepartureCompletedBottomSheet extends StatefulWidget {
  const NormalDepartureCompletedBottomSheet({super.key});

  @override
  State<NormalDepartureCompletedBottomSheet> createState() => _NormalDepartureCompletedBottomSheetState();
}

class _NormalDepartureCompletedBottomSheetState extends State<NormalDepartureCompletedBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final baseList = normalPlateState.normalGetPlatesByCollection(
    ...
    final normalSelectedPlate = normalPlateState.normalGetSelectedPlate(PlateType.departureCompleted, userName);
    final plateNumber = normalSelectedPlate?.plateNumber ?? '';
                    ...
                    return Scaffold(
                      ...
                      body: Column(
                        children: [
                          ...
                          const SizedBox(height: 8),
                          NormalDepartureCompletedSelectedDateBar(visible: !isSettled),
                          const SizedBox(height: 8),
                          Expanded(
                            child: TabBarView(
                              children: [
                                NormalDepartureCompletedUnsettledTab(
                                  firestorePlates: firestorePlates,
                                  userName: userName,
                                ),
                                NormalDepartureCompletedSettledTab(
                                  area: area,
                                  division: division,
                                  selectedDate: selectedDate,
