...
import 'normal_departure_completed_plate_image_dialog.dart';
...
/// === 위젯 ===
class NormalMergedLogSection extends StatefulWidget {
  final List<Map<String, dynamic>> mergedLogs; // 시그니처 호환용(내부 미사용)
  final String division;
  final String area;

  const NormalMergedLogSection({
    super.key,
  ...
  @override
  State<NormalMergedLogSection> createState() => _NormalMergedLogSectionState();
}

class _NormalMergedLogSectionState extends State<NormalMergedLogSection> {
  // 날짜 범위(기본: 최근 7일)
  ...
  void _openPlateImageDialog(String plateNumber) {
    try {
      showGeneralDialog(
        ...
        pageBuilder: (_, __, ___) => NormalDepartureCompletedPlateImageDialog(plateNumber: plateNumber),
      );
    } catch (e) {
      _logApiError(
        tag: '_NormalMergedLogSectionState._openPlateImageDialog',
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      await _logApiError(
        tag: '_NormalMergedLogSectionState._load',
  ...
  Future<void> _search() async {
    ...
    try {
      ...
      await _logApiError(
        tag: '_NormalMergedLogSectionState._search',
        message: '검색 처리 실패',
        error: e,
              ...
              child: IconButton(
                ...
                onPressed: () {
                  ...
                  try {
                    ...
                    _logApiError(
                      tag: '_NormalMergedLogSectionState.build.togglePage',
