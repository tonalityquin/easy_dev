...
import 'departure_completed_plate_search_bottom_sheet/normal_departure_completed_search_bottom_sheet.dart';
...
/// - 하단: "데이터 갱신" + "번호판 검색"
/// - Pull-to-refresh 지원
class NormalDepartureCompletedUnsettledTab extends StatefulWidget {
  const NormalDepartureCompletedUnsettledTab({
    super.key,
    required this.firestorePlates, // 시그니처 호환(실제 렌더링은 NormalPlateState 최신 데이터 사용)
  ...
  @override
  State<NormalDepartureCompletedUnsettledTab> createState() => _NormalDepartureCompletedUnsettledTabState();
}

class _NormalDepartureCompletedUnsettledTabState extends State<NormalDepartureCompletedUnsettledTab> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // 이미 로딩 중이면 중복 호출 방지
      if (!normalPlateState.isLoadingType(PlateType.departureCompleted)) {
        await normalPlateState.normalRefreshType(PlateType.departureCompleted);
      }
    });
  }
  ...
  Future<void> _openPlateSearchBottomSheet() async {
    ...
    await showModalBottomSheet(
      ...
      builder: (_) => NormalDepartureCompletedSearchBottomSheet(
        area: area,
  ...
  /// ✅ 데이터 갱신 로직: NormalPlateState에 출차완료 1회 재조회 요청
  Future<void> _refreshUnsettled() async {
    ...
    final lastAt = normalPlateState.normalLastRefreshAtOf(PlateType.departureCompleted);
    final sourceLabel = normalPlateState.normalLastRefreshSourceLabelOf(PlateType.departureCompleted);
  ...
  Widget build(BuildContext context) {
    ...
    final raw = normalPlateState.dataOfType(PlateType.departureCompleted);
    ...
    final lastRefreshAt = normalPlateState.normalLastRefreshAtOf(PlateType.departureCompleted);
    final sourceLabel = normalPlateState.normalLastRefreshSourceLabelOf(PlateType.departureCompleted);

    final bool isRefreshing = normalPlateState.isLoadingType(PlateType.departureCompleted);
