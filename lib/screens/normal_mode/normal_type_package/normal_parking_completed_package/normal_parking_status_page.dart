...
import 'normal_parking_reminder_contents.dart';
...
class NormalParkingStatusPage extends StatefulWidget {
  final bool isLocked;

  const NormalParkingStatusPage({super.key, required this.isLocked});

  @override
  State<NormalParkingStatusPage> createState() => _NormalParkingStatusPageState();
}

class _NormalParkingStatusPageState extends State<NormalParkingStatusPage> {
  ...
  Future<void> _runAggregateCount() async {
    ...
    try {
      ...
      // ✅ DebugApiLogger 로깅
      await _logApiError(
        tag: 'NormalParkingStatusPage._runAggregateCount',
  ...
  Future<void> _runNoticeFetch({required bool forceRefresh}) async {
    ...
    try {
      final result = await NormalParkingNoticeService.fetchNoticeMessage(
        area: area,
      ...
      // 호출부에서도 방어 + 로깅
      await _logApiError(
        tag: 'NormalParkingStatusPage._runNoticeFetch',
...
/// ✅ 상단 알림바(관리자 공지)
class _NormalParkingNoticeBar extends StatelessWidget {
  final bool isLoading;
  final String message;
  final VoidCallback onRefresh;

  const _NormalParkingNoticeBar({
...
/// ✅ Google Sheets API 기반 공지 서비스 (Normal)
class NormalParkingNoticeService {
  NormalParkingNoticeService._();
  ...
  static Future<sheets.SheetsApi> _sheetsApi() async {
    try {
      ...
      await _logApiError(
        tag: 'NormalParkingNoticeService._sheetsApi',
  ...
  static Future<String> _loadSpreadsheetId() async {
    try {
      ...
      await _logApiError(
        tag: 'NormalParkingNoticeService._loadSpreadsheetId',
  ...
  static Future<String> fetchNoticeMessage({
    ...
    final cacheKey = 'normal_parking_notice_cache_v2_${trimmedArea.isEmpty ? 'empty' : trimmedArea}';
    final cacheAtKey = 'normal_parking_notice_cache_at_v2_${trimmedArea.isEmpty ? 'empty' : trimmedArea}';
    final cacheSidKey = 'normal_parking_notice_cache_sid_v2_${trimmedArea.isEmpty ? 'empty' : trimmedArea}';
    ...
    if (spreadsheetId.isEmpty) {
      ...
      if (fallback.isEmpty) {
        await _logApiError(
          tag: 'NormalParkingNoticeService.fetchNoticeMessage',
