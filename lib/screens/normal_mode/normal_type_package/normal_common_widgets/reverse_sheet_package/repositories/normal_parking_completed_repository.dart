...
class NormalParkingCompletedRepository {
  final ParkingCompletedDb _dbProvider;
  NormalParkingCompletedRepository({ParkingCompletedDb? dbProvider})
      : _dbProvider = dbProvider ?? ParkingCompletedDb.instance;

  Future<int> insert(NormalParkingCompletedRecord record) async {
  ...
  Future<List<NormalParkingCompletedRecord>> listAll({
    int limit = 500,
    ...
    return rows.map((m) => NormalParkingCompletedRecord.fromMap(m)).toList();
  }
