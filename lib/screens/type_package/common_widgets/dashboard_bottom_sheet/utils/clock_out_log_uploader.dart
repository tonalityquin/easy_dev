...
// ✅ UserState / AreaState 사용 (출근 로직과 동일한 출처)
...
class ClockOutLogUploader {
  ...
  static Future<SheetUploadResult> uploadLeaveJson({
    ...
    try {
      // ✅ 출근 로직과 동일하게 UserState / AreaState 에서 정보 읽기
      ...
      if (spreadsheetId == null || spreadsheetId.isEmpty) {
        const msg = '퇴근 기록 업로드 실패: 출근부 스프레드시트 ID(commute_sheet_id)가 설정되지 않았습니다.\n'
            '관리자에게 출근부 스프레드시트 ID를 설정해 달라고 요청해 주세요.';
        debugPrint('❌ $msg');

        // 🔴 설정 누락도 운영 관점에서 중요한 실패이므로 에러 로그 남김
        await DebugApiLogger().log(
          {
            'tag': 'ClockOutLogUploader.uploadLeaveJson',
            'message': '퇴근 기록 업로드 실패 - 출근부 스프레드시트 ID(commute_sheet_id) 미설정',
          ...
          level: 'error',
          tags: const ['sheets', 'commute', 'clock_out'],
        );
      ...
      if (userId.isEmpty || userName.isEmpty || area.isEmpty || division.isEmpty || recordedTime.isEmpty) {
        final msg = '퇴근 기록 업로드 실패: 필수 정보가 비어 있습니다.\n'
      ...
      // 4) 업로드 (출근/휴게와 동일한 컬럼 구조)
      // A: 날짜, B: 시간, C: userId, D: 이름,
      // E: area(selectedArea), F: division, G: 상태(퇴근)
      final row = <Object?>[
        dateStr,
        recordedTime,
        userId,
      ...
      final msg = '퇴근 기록 업로드 중 오류가 발생했습니다.\n'
  ...
  static Future<List<List<String>>> _loadAllRecords(
    ...
    return result.values?.map((row) => row.map((cell) => cell.toString()).toList()).toList() ?? <List<String>>[];
  }
}
