...
/// SQLite 전용: 미정산 탭
/// - 선택한 날짜/현재 지역 기준으로 status_type='departureCompleted' 이면서 is_locked_fee=0 인 목록 표시
...
class _DepartureCompletedUnsettledTabState extends State<DepartureCompletedUnsettledTab> {
  ...
  Future<List<Map<String, Object?>>> _loadUnsettledRows() async {
    final db = await OfflineAuthDb.instance.database;

    // 선택한 날짜의 00:00 ~ 23:59 범위(ms)
    ...
    final rows = await db.query(
      ...
      ''',
      whereArgs: ['departureCompleted', widget.area, from, to],
      orderBy: 'COALESCE(request_time, COALESCE(updated_at, created_at)) DESC',
      limit: 300,
    );
              ...
              children: [
                ...
                _CollapsibleCard(
                  ...
                  child: const Padding(
                    ...
                    child: Text('상단 날짜는 상위 시트에서 선택되었습니다.', style: TextStyle(color: Colors.black54)),
                  ),
                ),
